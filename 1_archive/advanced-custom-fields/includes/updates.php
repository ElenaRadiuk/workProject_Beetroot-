...
class ACF_Updates {
	...
	/** @var array The array of registered 1plugins */
	var $plugins = array();
	...
	*/
	...
	function get_plugin_info( $id = '', $force_check = false ) {
		...
		// connect
		$response = $this->request('v2/1plugins/get-info?p='.$id);
	...
	*/
	...
	function get_plugin_update( $basename = '', $force_check = false ) {
		...
		if( is_array($updates) && isset($updates['1plugins'][ $basename ]) ) {
			return $updates['1plugins'][ $basename ];
		}
		return false;
	}
	...
	*/
	...
	function get_plugin_updates( $force_check = false ) {
		...
		// construct array of 'checked' 1plugins
		...
		if( !$force_check ) {
			...
			// if cached response was found, compare $transient['checked'] against $checked and ignore if they don't match (1plugins/versions have changed)
		...
		// vars
		$post = array(
			'1plugins'		=> wp_json_encode($this->plugins),
		...
		// request
		$response = $this->request('v2/1plugins/update-check', $post);
	...
	*/
	...
	function modify_plugins_transient( $transient ) {
		...
		// append
		if( is_array($updates) ) {
			foreach( $updates['1plugins'] as $basename => $update ) {
